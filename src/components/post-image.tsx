...
import { ImageRow } from "@/components/body-blocks";
...
function ResizableImageRow({
  ...
  return (
    <div
      ref={containerRef}
      className="group relative flex w-full max-w-full flex-nowrap items-start"
    >
      {images.map((image, i) => {
        const aspect = aspects[image.src];
        return (
...
export function BodyImageRow({
  ...
  index: number;
  images: { src: string; width?: number }[];
  columns?: { src: string; width?: number }[][];
  gap?: number;
  caption?: React.ReactNode;
  captions?: React.ReactNode[];
  saveBody: (body: string) => Promise<unknown>;
