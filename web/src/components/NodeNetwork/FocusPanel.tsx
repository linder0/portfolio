...
import { getCategoryColor } from "@/lib/graphLayout";
...
const TweetEmbed = ({ tweetId, label }: { tweetId: string; label: string }) => {
...
const StickyHeader = ({
  ...
  <motion.div
    ...
    style={{
      backgroundColor: "color-mix(in srgb, var(--theme-accent) 65%, transparent)",
...
export default function FocusPanel({ project, onClose }: FocusPanelProps) {
  ...
  return (
    <motion.div
      ...
      className={`absolute z-[60] ${
        isMobile ? "inset-0" : "top-0 right-0 w-full max-w-md h-full border-l"
      }`}
      style={{
      ...
      <div ref={panelRef} className="h-full overflow-y-auto">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {project.featured && <span className="text-sm opacity-60">★</span>}
          </div>
            ...
            {project.tools && project.tools.length > 0 && (
              <div className="flex flex-wrap items-center gap-2">
                {project.tools.map((tool) => (
                  <TagPill key={tool} variant="badge">
                    {tool}
                  </TagPill>
                ))}
              </div>
                ...
                <div key={i} className="space-y-1">
                  ...
                  >
                    <source
                      src={track.url}
                      type={
                        track.url.endsWith(".wav") ? "audio/wav" : "audio/mpeg"
                      }
                    />
