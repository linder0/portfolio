...
const EDGE_FADE_CLASS =
  "absolute inset-y-0 w-8 sm:w-16 md:w-40 z-20 pointer-events-none transition-theme";
...
export default function MediaPipeCanvas({
  ...
  // Cache the resolved --text color; only re-read on theme change.
  useEffect(() => {
  ...
  function initializeLetters(width: number, height: number) {
  ...
  return (
    <div
      ref={rootRef}
      className={`relative w-full h-[65vh] min-h-[300px] lg:h-full overflow-hidden bg-theme transition-theme ${className}`}
    >
